...
#[tauri::command]
#[specta::specta]
pub async fn backup_before_update(state: State<'_, AppState>) -> Result<(), String> {
    let backup_dir = state.paths.data_dir.join("backups");
    ...
    // Back up database
    let db_src = &state.paths.db_file;
    if db_src.exists() {
    ...
    // Back up settings
    let settings_src = &state.paths.settings_file;
