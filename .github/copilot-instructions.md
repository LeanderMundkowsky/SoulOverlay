...
├── bindings.ts                      # AUTO-GENERATED by tauri-specta — manual edits allowed only as dev-time unblock (overwritten on next debug build)
├── components/
...
**`hide_untranslated_items`** (default: `true`) — filters items whose name starts with `@` from
all search results and inventory add/edit dialogs. These are items with missing localization
keys in the wiki (e.g. `@items_commodities_valakkarpearl_apex_tier4`). Filtering happens on the
Rust side in `wiki_search` and `api_search_items`.
...
- Do NOT edit `src/bindings.ts` — it is auto-generated by tauri-specta on every debug build.
  **Exception**: when a Rust IPC type changes and `npm run tauri dev` can't be run to regenerate,
  apply the minimal manual edit to unblock `vue-tsc` (e.g. add the new field inline), then it
  will be overwritten correctly on the next debug-mode run.
...
**Item search deduplication** (applied in `wiki_search` and `api_search_items`):
- `_hair_extension` class variants: the wiki stores hairstyle-compatible armor/helmet variants
  under the same name as the base item with a `_hair_extension` suffix on `class_name`. These
  are filtered out entirely, but if a variant with that suffix is the only one with prices in
  the item prices cache, it is kept in preference to the base item.
- Same-name duplicates: deduplicated by name — priority: (1) has prices in cache, (2) not a
  `_hair_extension`, (3) first seen.
- Items with names starting with `@` are untranslated (missing localization keys). They are
  filtered out when `settings.hide_untranslated_items = true` (default: on).

**`UexResult.id` vs `UexResult.uuid` for items** — these can differ:
- `uuid` = Wiki API UUID (used for `wiki_entity_specs` and `api_entity_info` lookups)
- `id` = price-lookup key (used for `api_item_prices`). Normally equal to `uuid`, but for
  items where `items_prices_all.item_uuid` is null, UEX stores prices under the numeric UEX
  item ID (e.g. `"4563"`). `wiki_search` resolves this via the **item name mapper**.

**Item name mapper** (`providers/items/provider.rs`):
- Built during `ItemPrices::refresh()`: iterates all `items_prices_all` rows, builds a
  `HashMap<normalized_name, entity_id>` (normalized = lowercase, collapsed whitespace).
- Stored in cache under `ITEM_NAME_MAPPER_KEY = "item_name_mapper"` with the same TTL as
  `ItemPrices`.
- `wiki_search` loads the mapper and for each wiki item without UUID-keyed prices, looks up the
  item name to get the numeric UEX ID as `UexResult.id`. This bridges items where UEX hasn't
  linked `item_uuid` yet.

**Vehicle deduplication** (applied in `VehiclesCatalog::refresh()`):
- `dedup_vehicles_by_uuid()` — removes pagination boundary duplicates (parallel page fetches
  can return the same vehicle twice when the dataset shifts between requests).
- `dedup_vehicles_by_name()` — removes store/edition variants: the wiki stores e.g.
  "Polaris" and "Polaris Collector Military" both under wiki name `"Polaris"` with different
  `class_name` suffixes. First-seen entry is kept (API returns base ship first).
