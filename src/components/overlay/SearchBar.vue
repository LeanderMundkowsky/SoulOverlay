...
<template>
  <div class="relative p-4">
    ...
    <div
      v-if="error"
      class="mt-2 text-red-400 text-xs px-1"
    >
      {{ error }}
    </div>
    ...
    <div
      ...
      class="absolute top-full left-4 right-4 mt-0 bg-[#1a1d24] border border-white/10 rounded-b-lg shadow-2xl max-h-64 overflow-y-auto z-50 -translate-y-1"
    >
      <button
    ...
    <div
      ...
      class="absolute top-full left-4 right-4 mt-0 bg-[#1a1d24] border border-white/10 rounded-b-lg px-4 py-3 text-white/40 text-sm -translate-y-1 z-50"
    >
