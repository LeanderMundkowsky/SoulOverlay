...
<template>
  <div class="flex flex-col h-full bg-[#111318] border-l border-white/10 text-xs font-mono">
    ...
    <!-- Error state -->
    <div v-if="error" class="p-4 text-red-400">
      {{ error }}
    </div>

    <!-- Loading -->
    <div v-else-if="!info" class="p-4 text-white/40">
      Loading...
    </div>
      ...
      <section>
        ...
        <div class="space-y-1">
          <div class="flex justify-between gap-4">
            ...
            <span :class="info.sc_detected ? 'text-green-400' : 'text-red-400'">
              {{ info.sc_detected ? "yes" : "no" }}
            </span>
          </div>
          <div class="flex justify-between gap-4">
            <span class="text-white/50">Focused</span>
            <span :class="info.sc_focused ? 'text-green-400' : 'text-white/60'">
              {{ info.sc_focused ? "yes" : "no" }}
            </span>
          </div>
          <div class="flex justify-between gap-4">
      ...
      <section>
        ...
        <div class="space-y-1">
          ...
          <div class="flex justify-between gap-4">
            ...
            <span :class="info.uex_api_key_set ? 'text-green-400' : 'text-yellow-400'">
              {{ info.uex_api_key_set ? "set" : "not set" }}
            </span>
          </div>
          <div class="flex justify-between gap-4 min-w-0">
            <span class="text-white/50 flex-shrink-0">Log Path</span>
            <span class="text-white/60 truncate text-right" :title="info.log_path ?? 'default'">
              {{ info.log_path ?? "default" }}
            </span>
          </div>
        </div>
      </section>
      ...
      <section>
        ...
        <div class="space-y-1">
          <div class="flex justify-between gap-4">
            ...
            <span :class="info.log_watcher_active ? 'text-green-400' : 'text-red-400'">
              {{ info.log_watcher_active ? "active" : "inactive" }}
            </span>
          </div>
          <div class="flex justify-between gap-4">
