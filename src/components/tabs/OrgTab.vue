      ...
      <template v-else>
        ...
        <div v-if="orgStore.userInvitations.filter(i => i.status === 'pending').length > 0" class="space-y-2">
          ...
          <div
            ...
            :key="inv.id"
            class="flex items-center justify-between bg-teal-500/5 border border-teal-500/20 rounded-lg px-4 py-3"
          >
            <div>
            ...
            <div class="flex gap-2">
              <button @click="acceptInvite(inv.id)" class="text-xs px-3 py-1.5 bg-teal-500/20 border border-teal-500/40 text-teal-300 rounded-lg hover:bg-teal-500/30 transition-colors">Accept</button>
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            ...
            <div class="flex gap-2">
              ...
              <button
                ...
                class="text-xs px-3 py-1 bg-teal-500/20 border border-teal-500/40 text-teal-300 rounded-lg hover:bg-teal-500/30 transition-colors"
              >+ Create Org</button>
            </div>
          </div>
          ...
          <div v-if="showApplyForm" class="bg-[#1a1d24] border border-white/10 rounded-lg p-4 space-y-3">
            ...
            <div>
              ...
              <input v-model="applyOrgId" type="number" placeholder="12345" class="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder-white/20 focus:outline-none focus:border-teal-500/50" />
            </div>
            <div>
              <label class="block text-xs text-white/40 mb-1">Message <span class="text-white/20">(optional)</span></label>
              <textarea v-model="applyMessage" rows="2" placeholder="Why do you want to join?" class="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder-white/20 focus:outline-none focus:border-teal-500/50 resize-none" />
            </div>
            ...
            <button @click="applyToOrg" :disabled="!applyOrgId.trim() || applying" class="text-xs px-4 py-2 bg-teal-500/20 border border-teal-500/40 text-teal-300 rounded-lg hover:bg-teal-500/30 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
          ...
          <div v-else class="grid grid-cols-1 gap-3">
            <button
              ...
              class="text-left bg-white/5 border border-white/10 rounded-xl p-4 hover:border-teal-500/30 hover:bg-teal-500/5 transition-colors group"
            >
              <div class="flex items-center justify-between">
